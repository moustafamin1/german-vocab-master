...
import { Play, Check, X, Shield, Search } from 'lucide-react';
...
export default function ConfigScreen({
    ...
    return (
        <div className="max-w-xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
            ...
            {recoveryFindings.length > 0 && (
                <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl space-y-3 animate-pulse">
                    <div className="flex items-center gap-2 text-rose-400">
                        <Shield className="w-5 h-5" />
                        <p className="text-sm font-bold uppercase tracking-wider">Potential Progress Found!</p>
                    </div>
                    <p className="text-xs text-rose-200/70 leading-relaxed">
                        We found {recoveryFindings.length} items in your storage that might be your lost progress.
                    </p>
                    <div className="flex gap-2">
                        <button
                            onClick={onOpenSettings}
                            className="text-[10px] font-bold bg-rose-500 text-white px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-rose-600 transition-colors"
                        >
                            <Search className="w-3 h-3" />
                            Go to Recovery
                        </button>
                        <p className="text-[10px] text-rose-400/80 italic self-center">Visit Settings > Recovery to restore your data.</p>
                    </div>
                </div>
            )}
